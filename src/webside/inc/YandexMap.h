...
/*! \class WYandexMap Wt/WYandexMap Wt/WYandexMap
 *  \brief A widget that displays a Yandex map.
 *
 * This widget uses the online Yandex Maps server to display a map. It
 * exposes a part of the Yandex maps API.
 *
 * To use the map on a public server you will need to obtain a
 * key. The widget will look for this key as the configuration
 * property <tt>"aandex_api_key"</tt>. If this configuration property
 ...
 * Contributed by: Richard Ulrich.
 */
class WT_API WYandexMap : public WCompositeWidget
{
public:
   ...
   */
  WYandexMap(WContainerWidget *parent = 0);
  ...
  /*! \brief Enables the YandexBar, an integrated search control, on the map.
   *
   ...
   */
  void enableYandexBar();

  /*! \brief Disables the YandexBar integrated search control.
   *
   * When disabled, the default Powered by Yandex logo occupies the
   * position formerly containing this control. Note that this control
   * is already disabled by default.
   */
  void disableYandexBar();
