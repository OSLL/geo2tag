...
if [ -e $CATALINA_HOME/webapps/${app} ]; then
	...
	ant -Dsubject "geo2tag-web - midnight build (success): build, test, deploy reports" send-report
else 
	date > ${dir_log}/deploy.log.txt
	...
	ant -Dsubject "geo2tag-web - midnight build (fail): build, test, deploy reports" send-report
fi 
